(function () {
  ...
  class Editor {
    ...
    _initIndicators() {
      ...
      Tag.addEventListeners(this._codeWrapper, {
        mouseover: () => this._clearCarouselTimer(),
        mouseout: ({ target, currentTarget }) => {
          // _codeWrapper가 아닌 하위 요소에서 일어난 mouseout 무시
          if (target !== currentTarget) {
            return;
          }
          // textarea에 focus된 상태에서 일어난 mouseout 무시
          if (document.activeElement.classList.contains('textarea-code')) {
            return;
          }
          this._setCarouselTimer(2000, 3000);
        }
    ...
    _setCarouselTimer(timeoutDelay, carouselDelay) {
      ...
      // timeoutDelay를 기다리는 상태임을 명시
      this._autoCarouselTimer = 'ready';

      this._delayTimer = setTimeout(() => {
        // 만약 timeoutDelay를 기다리는 동안에 autoCarouselTimer가 변했다면 타이머를 set하지 않음
        // (ex codeWrapper에서 codeWrapper 안쪽 하위요소로 커서를 이동하면서 mouseout 된 경우,
        // autoCarouselTimer는 ready가 되고 바로 하위요소의 mouseover 이벤트가 발생하며 다시 null이 된다.)
        if (this._autoCarouselTimer !== 'ready') {
          return;
        }
